...
@section('content')
    <div id="content" class="content">
        ...
        <ol class="breadcrumb pull-right">
            ...
            <li><a href="javascript:;">Home Page Image</a></li>
            <li class="active">Manage Home Page Image</li>
        </ol>
        <!-- end breadcrumb -->
        <!-- begin page-header -->
        <h1 class="page-header">Manage Home Page Image</h1>
        <!-- end page-header -->
        <!-- begin row -->
        <div class="row">
            <!-- begin col-12 -->
                ...
                <div class="panel panel-inverse">
                    ...
                    <div class="panel-body">
                        <table id="data-table" class="table table-striped table-bordered">
                            ...
                            <tbody>
                            <?php $i=0?>
                            @foreach($homepageimages as $user)
                                <?php $i++;
                                if($i%2==0){
                                    $class='even gradeC';
                                }else{
                                ...
                                <tr class="{{$class}}">
                                    <td>{{$i}}</td>
                                    <td><img src="{{url('/')}}/Image/{{isset($user['image']) ? $user['image'] : ''}}" style="height: 80px;width: 80px"></td>
                                    <td>
                                    <span id="status{{$user->id}}">
                                        ...
                                        <a href="{{route('admin::editHomePageImage',['id'=>$user->id])}}" class="fancybox fancybox.iframe btn btn-primary"><span class="glyphicon glyphicon-edit"></span></a>&emsp;
                                        <a data-toggle="modal" data-target="#confirmDelete" class="btn btn-danger" onclick="getDeleteRoute('{{route('admin::deleteHomePageImage',['id'=>$user->id])}}')"><span class="glyphicon glyphicon-trash"></span></a>&emsp;
                                    </td>
                                </tr>
                            @endforeach
                            </tbody>
    ...
    @push('scripts')
        <script>
            ...
            function active_inactive_user(id,status) {
                $.ajax({
                    type: "post",
                    url: '{{route('admin::statusHomePageImage')}}',
                    data: {
